        ...
        <x-modal>
            <x-slot name="trigger">
                <button wire:click="storeModal"
                    ...
                    <i class="fas fa-plus mt-1 mr-1"></i> Add a new Point of interest
                </button>
            </x-slot>
            <x-slot name="title">Create a new Point of interest</x-slot>
            <x-slot name="content">
                <x-admins.poi-form :updating="0" />
            </x-slot>
            <x-slot name="Action">
                <button type="button"
                    class="bg-green-500 hover:bg-green-600 px-4 py-2 rounded text-white focus:outline-none"
                    ...
                    @foreach ($countryPois as $poi)
                        <tr class="border-b hover:bg-orange-100" :class="{ ' bg-gray-200' : {{ $loop->index }}}">
                            ...
                            <td class="p-3 px-5 flex flex-col sm:flex-row justify-end">
                                ...
                                <x-modal class="mb-2">
                                    <x-slot name="trigger">
                                        <button type="button" wire:click="updateModal({{ $poi->id }})"
                                    ...
                                    <x-slot name="title">update {{ $poi->name }}</x-slot>
                                    <x-slot name="content">
                                        <x-admins.poi-form :updating="1" />
                                    </x-slot>
                                    <x-slot name="Action">
                                        <button
                                            class="bg-green-500 hover:bg-green-600 px-4 py-2 rounded text-white focus:outline-none "
                                ...
                                <x-modal class="mb-2 mr-2">
                                    ...
                                    <x-slot name="title">Delete Point of interest</x-slot>
                                    <x-slot name="content">
                                        <div class="flex p-6">
                                    ...
                                    <x-slot name="Action">
                                        <button wire:click="Delete({{ $poi->id }})"
                                ...
                                <x-modal class="mb-2">
                                    <x-slot name="trigger">
                                        <button type="button" wire:click="POIImages({{ $poi->id }})"
                                    ...
                                    <x-slot name="title">{{ $poi->name }} images</x-slot>
                                    <x-slot name="content">
                                        @if ($POIImages)
                                            @foreach ($POIImages as $image)
                                                <div class="mb-2 flex justify-center items-start">
                                                    <img src="{{ asset('storage/POI/' . $image->file_name) }}" alt=""
                                                        class=" w-10/12 h-full mb-2 ">
