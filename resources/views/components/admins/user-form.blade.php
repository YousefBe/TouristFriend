<div >
    <div class="bg-white px-3 py-4 rounded text-black w-full" > 
        ...
        <input 
            type="text"
            class="block border border-grey-light p-3 rounded mb-4"
            name="fullname"
        ...
        <div class="grid grid-cols-2 sm:grid sm:grid-cols-4 ">
            @foreach ($permissions as $permission)
               <label class="inline-flex items-center mt-3">
                <input type="checkbox" wire:model.lazy="userPermissions" value="{{$permission->id}}"
                wire:click="permissionChecked({{$permission->id}})" id="{{ rand() }}"
                class="form-checkbox h-5 w-5 text-gray-600"><span class="ml-2 text-gray-700 text-sm">{{$permission->display_name}}</span>
               </label>
            @endforeach
        </div>
    </div>
